...
def symmetric_normalization_coefficients(edges: torch.LongTensor) -> torch.FloatTensor:
    deg = node_degree_from_edge_list(edges).float()
    deg_inv_sqrt = deg.pow_(-0.5)
    return deg_inv_sqrt[edges[0, :]] * deg_inv_sqrt[edges[1, :]]

class EdgeList:

    def __init__(self, edges: torch.LongTensor):
        self.edges = edges

    @cached_property
    def symmetric_norm_coefficients(self):
        return 